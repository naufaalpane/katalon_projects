use IPPCS_QA
 ...
 declare @@PID                             varchar(max)            =    @PID                                       --  202101280366            PID = PID,
 declare @@dockServicePart 		         varchar(max)			   =    @dockServicePart 		                    --'S1, C9, G1'			   dockServicePart = DOCK_SERVICE_PART,
 declare @@TP_SUPPLIER_CODE 	           varchar(max)			   =    @TP_SUPPLIER_CODE 		                    --'807%'				   TP_SUPPLIER_CODE = TP_SPLR_CODE,
 declare @@SYS_SOURCE 			         varchar(max)			   =    @SYS_SOURCE 			                    --'IPPCS'				   SYS_SOURCE = SYSTEM_SOURCE_GR,
 declare @@PROD_PURPOSE_CD 		         varchar(max)			   =    @PROD_PURPOSE_CD 		                    --'D'					   PROD_PURPOSE_CD = PROD_PURPOSE_CD_GR,
 declare @@prodPurposeServicePart               varchar(max)	   =    @prodPurposeServicePart                    --'S'					   prodPurposeServicePart = PROD_PURPOSE_GR_SERVICE_PART,
 declare @@SOURCE_TYPE_TP 		         varchar(max)			   =    @SOURCE_TYPE_TP 		                    --'3'					   SOURCE_TYPE_TP = SOURCE_TYPE_TP,
 declare @@MAT_DOC_DESC 		           varchar(max)            =    @MAT_DOC_DESC 			                    --'Test'				   MAT_DOC_DESC = MAT_DOC_DESC,
 declare @@URI_UOM 				         varchar(max)			   =    @URI_UOM 				                    --'PC'					   URI_UOM = ORI_UNIT_MEASU_CD_GR,
 declare @@FLTR_DATETIME_VALUE 	                 varchar(max)	   =    @FLTR_DATETIME_VALUE 	                    --'20120319180000'		   FLTR_DATETIME_VALUE = POST_TIME_FLTR,
 declare @@FLTR_LOGICAL_VALUE 	                 varchar(max)	   =    @FLTR_LOGICAL_VALUE 	                    --'<='					   FLTR_LOGICAL_VALUE = POST_LGCL_FLTR,
 declare @@FLTR_FLAG_VALUE		         varchar(max)			   =    @FLTR_FLAG_VALUE		                    --'N'					   FLTR_FLAG_VALUE = POST_FLG_FLTR,
 declare @@TYPE_CODE 			         varchar(max)			   =    @TYPE_CODE 			                         --'0'					   TYPE_CODE = TYPE_CODE,
 declare @@clientId 			           varchar(max)            =    @clientId 				                    --'301'					   clientId = clientId
...
left(PART_NO,10) as SND_PART_NO,
PROD_PURPOSE_CD as SND_PROD_PURPOSE_CD,
SOURCE_TYPE as SND_SOURCE_TYPE,
(select top 1 PLANT_CD from TB_M_DOCK_ICS where DOCK_CD = SHIPPING_DOCK and DROP_STATUS_DATE between VALID_DT_FR and VALID_DT_TO) as SND_PLANT_CD,
(select top 1 SLOC_CD from TB_M_DOCK_ICS where DOCK_CD = SHIPPING_DOCK and DROP_STATUS_DATE between VALID_DT_FR and VALID_DT_TO) as SND_SLOC_CD,
'' as SND_BATCH_NO,
left(PART_NO,10) as RCV_PART_NO,
PROD_PURPOSE_CD as RCV_PROD_PURPOSE_CD,
SOURCE_TYPE as RCV_SOURCE_TYPE,
(select top 1 PLANT_CD from TB_M_DOCK_ICS where DOCK_CD = DOCK_CODE and DROP_STATUS_DATE between VALID_DT_FR and VALID_DT_TO) as RCV_PLANT_CD,
(select top 1 SLOC_CD from TB_M_DOCK_ICS where DOCK_CD = DOCK_CODE and DROP_STATUS_DATE between VALID_DT_FR and VALID_DT_TO) as RCV_SLOC_CD,
'' as RCV_BATCH_NO,
