...
using GPPSU.Models.OrderRecTentativeNQCInhouse;
...
{
    public class OrderRecTentativeNQCInhouseController : BaseController
    {
        public OrderRecTentativeNQCInhouseRepo ordRecTenNQCRepo = OrderRecTentativeNQCInhouseRepo.Instance;

        protected override void Startup()
        {
            Settings.Title = "WA0AUE12 - Order Rec. Tentative NQC (Inhouse)";
        }

        public ActionResult executeBatch(OrderRecTentativeNQCInhouse data)
        {
            IDBContext db = DatabaseManager.Instance.GetContext();

            try
            {
                var returnStatus = ordRecTenNQCRepo.OrderCekTenNQC(db,data.COMPANY_CD, data.VERSION, GetLoginUserId());
            ...
            catch (Exception ex)
            {
                ...
                return ShowErrorMessages(ex.Message);
            }
        }

        public string downloadWarningList(OrderRecTentativeNQCInhouse data)
        {
            ...
            try
            {
                string fileName = "Order Warning List.pdf";
                ...
                //resultBatch = ordRecTenNQCRepo.recordToBatchStatus(db, data.COMPANY_CD, "OK");
